...
export default class Pokemon extends Component {
  ...
  async carregarListaPokemons() {
    let response = await pokeApi.get("pokemon?limit=721&offset=0");
    let arrayPokemon = [];
        ...
        {this.state.pokemons && this.state.pokemons.length > 0 ? (
          <div className="container">
            {this.state.pokemons.map((pokemon, index) => (
              ...
              to={{
                pathname: "/pokeinfos",
                state: { detalhesPokemon: pokemon, paginaAnterior: "/" }
              }}
            >
              <section key={index}>
                <div className="card" id={pokemon.name} style={{background: `${TYPE_COLORS[pokemon.types]}` }} >
